...
describe('crypto — PBKDF2 + AES-GCM round-trip', () => {
  test('encrypt / decrypt round-trips a payload', async () => {
    const key = await deriveKey('WARM-KITCHEN-LOAF');
    const plaintext = new TextEncoder().encode('hello kitchen');
    const frame = await encrypt(key, plaintext);
    const recovered = await decrypt(key, frame);
    expect(new TextDecoder().decode(recovered)).toBe('hello kitchen');
  });

  test('different phrases derive different keys (decrypt fails)', async () => {
    const k1 = await deriveKey('WARM-KITCHEN-LOAF');
    const k2 = await deriveKey('SOFT-KETTLE-JAM');
