 ...
 *
 * Hearth is N-party: housemates can join over time (a new flatmate
 * moves in two months after pairing). The SignalRoom DO already
 * fans out to every connected peer in the room, so we just bind the
 * Y.Doc and let the CRDT converge.
 *
 * Architecture:
...
const log = (...args: unknown[]) => console.info('[hearth:relay]', ...args);
...
export interface RelayState {
  status: RelayStatus;
  /** Number of OTHER housemates currently in the room (excludes self). */
  peerCount: number;
