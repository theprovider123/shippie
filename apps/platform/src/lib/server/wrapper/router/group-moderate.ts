/**
 * /__shippie/group/{id}/moderate — group moderation panel.
 * Ported from services/worker/src/router/group-moderate.ts.
 *
 * Owner-only client-side; the wrapper just serves a fixed HTML page.
 */
import type { WrapperContext } from '../env';

export function handleGroupChooser(ctx: WrapperContext): Response {
  return new Response(renderChooser(ctx.slug), {
    status: 200,
    headers: { 'Content-Type': 'text/html; charset=utf-8' }
  });
}

export function handleGroupModerate(
  ctx: WrapperContext,
  groupId: string
): Response {
  return new Response(renderModerate(ctx.slug, groupId), {
    status: 200,
    headers: { 'Content-Type': 'text/html; charset=utf-8' }
  });
}

function renderModerate(slug: string, groupId: string): string {
  return `<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8" />
...
function renderChooser(slug: string): string {
  return `<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8" />
