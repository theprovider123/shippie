/**
 * /maker/apps/[slug]/settings — SDK setup, deploy history, App Kind,
 * PWA readiness, and advanced operational settings.
 */
...
export const load: PageServerLoad = async ({ parent, platform }) => {
  ...
  const kindProfile = platform.env.CACHE ? await readAppKindProfile(platform.env.CACHE, layout.app.slug) : null;
  const override = platform.env.CACHE ? await readShippieJsonOverride(platform.env.CACHE, layout.app.slug) : null;
  const workflowProbes = Array.isArray(override?.workflow_probes) ? (override?.workflow_probes as string[]) : [];
...
async function setKindStatus(
  ...
  const db = getDrizzleClient(platform.env.DB);
  await db.update(schema.apps).set({ currentPublicKindStatus: newStatus }).where(eq(schema.apps.id, appId));

  if (platform.env.CACHE) {
    const profile = (await readAppKindProfile(platform.env.CACHE, slug)) as AppKindProfile | null;
    if (profile) {
...
export const actions: Actions = {
  ...
  saveWorkflowProbes: async ({ request, locals, platform, params, url }) => {
    ...
    const existing = (await readShippieJsonOverride(platform.env.CACHE, params.slug!)) ?? {};
