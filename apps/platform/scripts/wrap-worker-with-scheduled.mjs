 ...
 * exports a `scheduled()` handler and platform Durable Object classes
 ...
 *   4. We bundle Durable Object entries into small sidecar modules so their
 *      classes are reachable from the Worker module entry.
 *   5. We emit a tiny `_worker.js` that re-exports `default` from the
 *      original sveltekit bundle, adds a `scheduled` from `_cron.js`, and
 *      re-exports `SignalRoom` and `BusPulseSegment` from those sidecars.
 *
...
const paradeEntry = resolve(root, 'src/lib/server/parade/bus-pulse.ts');
const paradeOut = resolve(dest, '_parade.js');
const assetsIgnore = resolve(dest, '.assetsignore');
...
await bundleEntry(paradeEntry, '_parade.js');
...
if (!existsSync(paradeOut) || statSync(paradeOut).size === 0) {
  console.error(`[wrap-worker] parade bundle missing or empty at ${paradeOut}`);
  process.exit(1);
}
writeFileSync(
  assetsIgnore,
...
console.log('[wrap-worker] wrote .assetsignore for worker module sidecars');
// Emit the wrapper.
