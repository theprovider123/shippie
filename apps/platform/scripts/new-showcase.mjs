...
console.log('→ showcase catalog and role registry will refresh on next prepare-showcases/build run');
 ...
 * version string-spliced state.ts, which targeted an old array shape and
 * collided destructively when multiple agents scaffolded apps at once.
 */
function curatedAppSpec(slug, name, short, desc, accent, port) {
  return `  {
    ...
    // Use the manifest intent vocabulary; @shippie/intents canonicalises
    // legacy aliases during validation.
...
SELECT lower(hex(randomblob(16))), '${slug}', ${sqlString(name)}, ${sqlString(desc)}, ${sqlString(desc)}, 'app', 'tools', '${accent}', '#FAF7EF', 'zip', 'static', users.id, 'public', 'labs', 0
