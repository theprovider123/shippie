/**
 * Local DB resolver. In production it comes from the Shippie runtime
 * (`window.shippie.local.db`). Standalone dev uses an in-memory mock so
 * the app boots without the worker.
 *
 * Mirrors the pattern used by showcase-recipe; copied here so atlas
 * doesn't pick up an inappropriate transitive dep.
 */
import type {
  LocalDbBackupInfo,
