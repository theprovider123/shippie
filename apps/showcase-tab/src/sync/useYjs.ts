 ...
 * remote/local change.
 *
 * useSyncExternalStore demands getSnapshot return the SAME reference
 * until something actually changed. We track a version counter that
 * bumps on each Y.Doc update, then memoise the latest computed
 * snapshot so React is happy.
 */
