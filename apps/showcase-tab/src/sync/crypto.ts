/**
 * E2E encryption for relay traffic in Tab.
 *
 * The N phones at the table derive an AES-GCM key from their shared
 * encryption phrase via PBKDF2. Every Yjs update sent over the
 * SignalRoom relay is encrypted with a fresh random 12-byte nonce.
 *
 * Threat model:
 * - The relay sees ciphertext + nonce, NEVER plaintext.
 * - Anyone at the table (anyone with the encryption phrase) can decrypt.
 ...
 *   the room code (e.g. from a glance across the restaurant) they can't
 *   decrypt the wire traffic without the phrase.
 * - Forward secrecy is NOT a goal; rotate by closing the tab and
 *   starting a new one.
 *
 * Tab-specific: salt is `tab:e2e:v1` so even if a phrase is accidentally
 * re-used between a Mevrouw couple and a Tab dinner, the derived keys
 * differ.
 */

const SALT = new TextEncoder().encode('tab:e2e:v1');
