/**
 * RelayProvider — Yjs N-party sync via the Shippie SignalRoom DO.
 *
 * Tab is N-party: any number of diners can join a tab over the course
 * of an evening (someone arrives late, two people split the bill, a
 * housemate joins the trip mid-week). The SignalRoom DO already fans
 * out to every connected peer in the room, so we just bind the Y.Doc
 * and let the CRDT converge.
 *
 * Architecture:
 *   1. Open WebSocket to `${origin}/__shippie/signal/<roomSlug>`.
 ...
 * encryption phrase never leaves the diners' phones.
 */
...
const log = (...args: unknown[]) => console.info('[tab:relay]', ...args);

export interface RelayProviderOptions {
  doc: Y.Doc;
  /** Slug used as the SignalRoom URL slug. */
  roomSlug: string;
  /** Plaintext encryption phrase — used only on this device for AES-GCM key derivation. */
  phrase: string;
...
export interface RelayState {
  status: RelayStatus;
  /** Number of OTHER diners currently in the room (excludes self). */
  peerCount: number;
...
export interface RelayProvider extends RelayState {
  ...
  /** Force-reconnect + push full state vector. */
  resync: () => void;
  destroy: () => void;
}
