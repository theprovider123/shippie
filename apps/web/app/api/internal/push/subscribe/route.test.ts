...
describe('POST /api/internal/push/subscribe', () => {
  const path = '/api/internal/push/subscribe';

  test('upserts a subscription row, then updates keys on conflict', async () => {
    ...
    const req1 = await signedRequest(path, body1);
    ...
    expect(rows[0]!.appId).toBe('zen-notes');
    expect((rows[0]!.keys as { p256dh: string }).p256dh).toBe('key-a');

    // Re-post with rotated keys — should update not duplicate.
    ...
    const req2 = await signedRequest(path, body2);
    ...
    rows = await db
      ...
      .where(eq(schema.wrapperPushSubscriptions.endpoint, 'https://push.example.com/sub/abc'));
    expect(rows).toHaveLength(1);
    expect((rows[0]!.keys as { p256dh: string }).p256dh).toBe('key-b');
  });
