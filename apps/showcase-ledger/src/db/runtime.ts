 ...
 *
 * Cloned from showcase-recipe/db/runtime.ts. The two showcases keep
 * their own copies on purpose — the runtime contract is exported by
 * @shippie/local-runtime-contract; the in-memory fallback is small
 * enough that a shared package isn't worth the friction.
 */
import type {
  LocalDbBackupInfo,
...
class MemoryLocalDb implements ShippieLocalDb {
  ...
  async vectorSearch<T extends LocalDbRecord = LocalDbRecord>(
    ...
    return all.slice(0, opts.limit ?? 10).map((row) => ({ ...row, score: 0 }));
  }
  ...
  async export(table: string, _opts: LocalDbExportOptions = {}): Promise<Blob> {
