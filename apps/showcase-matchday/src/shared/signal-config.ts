...
import type { RoomParams, Role } from './types.ts';
...
export function readRoomParams(): RoomParams {
  ...
  if (typeof window === 'undefined') {
    return { role: null, roomId: null, signalBase: defaultSignalBase, roomKey: null };
  }
  ...
  return { role, roomId, signalBase, roomKey };
}
...
export function matchdayUrl(input: {
  role: Role;
  roomId: string;
  roomKey: string;
  signalBase?: string;
}): string {
  const url = new URL(typeof location === 'undefined' ? 'https://shippie.app/run/matchday/' : location.href);
