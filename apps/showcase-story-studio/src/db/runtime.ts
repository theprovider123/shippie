 ...
 * (`window.shippie.local.db`). Standalone dev + tests fall back to an
 * in-memory implementation so the app boots without the worker.
 *
 * Mirrors apps/showcase-recipe/src/db/runtime.ts on purpose — same
 * contract, same fallback shape — so future runtime work stays
 * consistent across showcases.
 */
import type {
  LocalDbBackupInfo,
...
export class MemoryLocalDb implements ShippieLocalDb {
  ...
  async vectorSearch<T extends LocalDbRecord = LocalDbRecord>(
    ...
    _opts: { limit?: number; column?: string } = {},
