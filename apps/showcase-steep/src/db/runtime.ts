 ...
 * (`window.shippie.local.db`). In dev/standalone we fall back to an
 * in-memory implementation so the app boots without the worker.
 *
 * Mirrors apps/showcase-recipe/src/db/runtime.ts deliberately — when
 * Steep + Recipe both prove out, the durability primitives lift to
 * @shippie/durability and this file collapses to a thin re-export.
 */
import type {
  LocalDbBackupInfo,
  LocalDbQueryOptions,
...
class MemoryLocalDb implements ShippieLocalDb {
  ...
  async count(table: string): Promise<number> {
    return this.must(table).rows.size;
  }

  async export(): Promise<Blob> {
    const dump: Record<string, LocalDbRecord[]> = {};
    for (const [name, table] of this.tables) {
      dump[name] = [...table.rows.values()];
    }
    return new Blob([JSON.stringify(dump)], { type: 'application/json' });
  }

  async restore(): Promise<LocalDbBackupInfo> {
    // Memory restore is intentionally a no-op for the contract — true
    // restoration is exercised through queries.ts wipe + re-insert
    // (see backup.recovery.test.ts). Returning a stub keeps the type
    // contract honest without committing to in-memory restore semantics.
    return {
