...
describe('steep data safety prompts', () => {
  it('records backup metadata in local storage', () => {
    ...
    expect(meta?.blendCount).toBe(3);
    expect(loadBackupMeta(storage)?.contentHash).toBe('abc');
    expect(storage.getItem(BACKUP_META_KEY)).toContain('2026-05-09');
  });

  it('prompts on first and fifth blend saves', () => {
    const storage = new TestStorage();
    const backup = loadBackupMeta(storage);

    expect(recordBlendSaveAndChoosePrompt(backup, new Date('2026-05-09'), storage)).toBe(
      'first-save',
    );
    expect(recordBlendSaveAndChoosePrompt(backup, new Date('2026-05-09'), storage)).toBeNull();
    expect(recordBlendSaveAndChoosePrompt(backup, new Date('2026-05-09'), storage)).toBeNull();
    expect(recordBlendSaveAndChoosePrompt(backup, new Date('2026-05-09'), storage)).toBeNull();
    expect(recordBlendSaveAndChoosePrompt(backup, new Date('2026-05-09'), storage)).toBe(
      'five-saves',
  ...
  it('startup prompt only fires for stale backups after real saves', () => {
    const storage = new TestStorage();
    expect(chooseStartupBackupPrompt(null, new Date('2026-05-09'), storage)).toBeNull();

    recordBlendSaveAndChoosePrompt(null, new Date('2026-05-09'), storage);
    saveBackupMeta(
      {
...
describe('deriveStorageStatus', () => {
  it('treats memory mode + critical iOS risk + critical quota as at-risk', () => {
    expect(
      deriveStorageStatus({
        mode: 'memory',
        persisted: true,
  ...
  it('promotes to safe only when persisted, recently backed up, no iOS risk', () => {
    expect(
      deriveStorageStatus({
        mode: 'opfs',
        persisted: true,
  ...
  it('falls back to only-on-this-device when persistence is not granted', () => {
    expect(
      deriveStorageStatus({
        mode: 'opfs',
        persisted: false,
...
describe('persistence metadata', () => {
  ...
  it('reads back what the runtime wrote, regardless of granted flag', () => {
    const storage = new TestStorage();
    storage.setItem(
      PERSISTENCE_META_KEY,
      JSON.stringify({ requestedAt: '2026-05-09T10:00:00.000Z', granted: false }),
    );
