 ...
 *
 * Therapy Notes never phones home — even in dev. The memory fallback
 * exists only so a developer can run the app standalone; on a real
 * device the data lives in OPFS via wa-sqlite.
 */
import type {
  LocalDbBackupInfo,
  LocalDbQueryOptions,
