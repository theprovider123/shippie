 ...
 * authorship tag on schedule entries, dose logs, and handover notes —
 * it survives across sessions and doesn't change.
 */
const STORAGE_KEY = 'co-pilot:pairing';

export type ParentRole = 'a' | 'b';

export interface Pairing {
  /** Human-readable pair code, e.g. "BIRCH-NORTH-3849". Generated on parent A's phone, entered on parent B's. */
  pairCode: string;
...
export function roomIdFor(pairCode: string): string {
  ...
  return `co-pilot-${(h >>> 0).toString(36)}`;
}
