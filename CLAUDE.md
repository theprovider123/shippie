...
- **SvelteKit route modules may only export the framework's allowed symbols — a bare helper export fails the BUILD (not typecheck/test).** `+page.server.ts` allows `load`/`actions`/`prerender`/`csr`/`ssr`/`trailingSlash`/`config`/`entries`; `+server.ts` allows HTTP verbs (`GET`/`POST`/…) plus `prerender`/`trailingSlash`/`config`/`entries`/`fallback`. Anything else must be `_`-prefixed OR live in a sibling module. A bare `export function foo` from a route file passes `tsc --noEmit` and `vitest` but dies at `bun run build` with `Invalid export 'foo'`. Put testable helpers in a colocated non-route file (e.g. `routes/admin/arcade-auto-lift.ts`) and import them into both the route and the test. (Bit twice on 2026-06-14.)
