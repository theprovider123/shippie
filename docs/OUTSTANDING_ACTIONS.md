...
- [x] **Phase 1 — Transformers runtime via esm.sh CDN (live).** `apps/platform/src/lib/container/ai-worker.ts` and the marketplace SW's `AI_RUNTIME_URLS` constant both point at `https://esm.sh/@huggingface/transformers@3.0.0` (pinned). esm.sh resolves the bare-specifier graph (onnxruntime-web, buffer, process) automatically with CORS + 1-year immutable cache headers; the SW pre-caches the shim on install. On-device AI calls now resolve `source: 'local'` instead of `'unavailable'` once the user's first AI invocation warms the transitive closure.
- [x] **Phase 2 — Self-host the Transformers runtime on shippie.app.** Same-origin proxy at `/__esm/<path>` (`apps/platform/src/routes/__esm/[...path]/+server.ts` + `apps/platform/src/lib/server/esm-proxy.ts`) mirrors the pinned esm.sh artifact, rewriting every absolute import in JS bodies back into the `/__esm/` namespace so the entire transitive graph (onnxruntime-web, buffer, process) stays on our zone after first parse. CF edge cache holds responses with `cache-control: public, max-age=31536000, immutable`. The marketplace SW pre-caches the entry URL on install and cache-firsts the whole `/__esm/*` namespace from `MODEL_CACHE`. AI worker now dynamic-imports `/__esm/@huggingface/transformers@3.0.0` instead of `https://esm.sh/...` — third-party runtime dependency eliminated. The `models.shippie.app` zone + R2 mirror is deferred (would only matter if we later need to ship a custom-trimmed bundle); same-origin proxy on shippie.app handles the headline goal without a new zone.
