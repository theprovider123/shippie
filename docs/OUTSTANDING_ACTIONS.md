...
- [ ] **Phase 2 — Self-host the Transformers runtime on `models.shippie.app`.** Replace the esm.sh CDN with a Shippie-controlled CF Worker that proxies the same artifact from R2. Reasons to do it: (a) eliminate third-party runtime dependency, (b) mirror with `cache-control: public, max-age=31536000, immutable` from our own zone, (c) opens the door to a custom-trimmed bundle (~2 MB) for the showcase task set instead of the full ~5–10 MB. Steps: create `models.shippie.app` zone in CF; create a small worker that fetches `https://esm.sh/@huggingface/transformers@<pin>` once into R2 then serves from there; flip the constants in `ai-worker.ts` + `sw.js/+server.ts`. ~1–2 hours; deferred until the runtime starts seeing real-device traction.
